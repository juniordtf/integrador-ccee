...
const listarParticipantesDeMercadoPorCnpj = async (
  authData,
  date,
  cnpj
): Promise<object> => {
   ...
   <soapenv:Body>
      <bm:listarParticipanteMercadoRequest>
          <bm:participantesMercado>
           ...
           <!-- <bm:nomeEmpresarial>NOME EMPRESARIAL</bm:nomeEmpresarial> -->
           <bm:periodoReferencia>
               <!-- <bo:inicio>2024-05-02T08:00:00</bo:inicio> -->  
           </bm:periodoReferencia>
       </bm:listarParticipanteMercadoRequest>
    </soapenv:Body>
  </soapenv:Envelope>`;
    ...
    api()
      ...
      .then((response) => {
        ...
        if (response.status === 200) {
          ...
          responseData = { data: cnpj, code: response.status };
          resolve(responseData);
        }
      })
      .catch(function (error) {
        if (error.response) {
          var responseData = { data: cnpj, code: error.response.status };
          console.log(error.response.status);
          resolve(responseData);
        }
      });
...
export const cadastrosService = {
  ...
  listarParticipantesDeMercadoPorCnpj,
  listarPerfis,
  listarRepresentacao,
};
